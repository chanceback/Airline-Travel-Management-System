...
function FlightsPage({ setFlightToUpdate }) {
    ...
    const onDeleteFlight = async id => {
        const response = await fetch(`${API_URL}/flights/${id}`, { method: 'DELETE' });
        if (response.status === 204) {
            const getResponse = await fetch(`${API_URL}/flights-table`);
            const flights = await getResponse.json();
            setFlights(flights);
        } else {
            console.error(`Failed to delete flight with id = ${id}, status code = ${response.status}`)
        }
    }
